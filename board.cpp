...
const int materialm[6] = {82, 337, 365, 477, 1025, 20000};
const int materiale[6] = {94, 281, 297, 512, 936, 20000};
const int pstm[6][64] = {{0,0,0,0,0,0,0,0,-35,-1,-20,-23,-15,24,38,-22,-26,-4,-4,-10,3,3,33,-12,-27,-2,-5,12,17,6,10,-25,
...
const int pste[6][64] = {{0,0,0,0,0,0,0,0,13,8,8,10,13,0,2,-7,4,7,-6,1,0,-5,-1,-8,13,9,-3,-7,-7,-8,3,-1,
...
const int castlechange[64] = {13, 15, 15, 15, 12, 15, 15, 14, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
...
std::string coordinate(int square) {
    std::string convert[64] = { "a1", "b1", "c1", "d1", "e1", "f1", "g1", "h1", "a2", "b2", "c2", "d2", "e2", "f2", "g2", "h2",
...
int Board::generatemoves(int color, bool capturesonly, int depth) {
    ...
    for (int i = 0; i < knightcount; i++) {
        int knightsquare = __builtin_popcountll((opponentknights & -opponentknights)-1);
        opponentattacks |= KnightAttacks[knightsquare];
        opponentknights^=(1ULL << knightsquare);
    }
    for (int i = 0; i < bishopcount; i++) {
        int bishopsquare = __builtin_popcountll((opponentbishops & -opponentbishops)-1);
    ...
    for (int i = 0; i < queencount; i++) {
        int queensquare = __builtin_popcountll((opponentqueens & -opponentqueens)-1);
    ...
    int opponentking = __builtin_popcountll((Bitboards[opposite]&Bitboards[7])-1);
...
int Board::evaluate(int color) {
    int midphase = std::min(24, gamephase[0]+gamephase[1]);
    int endphase = 24-midphase;
    int mideval = evalm[color]-evalm[color^1];
    int endeval = evale[color]-evale[color^1];
    int bishops = 37*(__builtin_popcountll(Bitboards[color]&Bitboards[4])/2-__builtin_popcountll(Bitboards[color^1]&Bitboards[4])/2);
    int e = (std::min(gamephase[0], gamephase[1]) == 0) ? 2 : 1;
...
bool Board::see_exceeds(int move, int color, int threshold) {
    ...
    pieces[0][0] = __builtin_popcountll((PawnAttacks[color][target]&Bitboards[2]&enemy));
    pieces[1][0] = __builtin_popcountll((PawnAttacks[color^1][target]&Bitboards[2]&us));
    ...
    occupied |= to;
    occupied &= (~from);
    pieces[0][3] = __builtin_popcountll((FileAttacks(occupied,target)|GetRankAttacks(occupied,target))&Bitboards[5]&enemy);
    occupied ^= (Bitboards[5]);
    occupied |= to;
    occupied &= (~from);
    pieces[1][3] = __builtin_popcountll((FileAttacks(occupied,target)|GetRankAttacks(occupied,target))&Bitboards[5]&us);
    occupied ^= (us&(Bitboards[5]|Bitboards[2]|Bitboards[4]));
    occupied |= to;
    occupied &= (~from);
    pieces[1][2] = __builtin_popcountll((DiagAttacks(occupied,target)|AntiAttacks(occupied,target))&Bitboards[4]&us);
    occupied ^= (Bitboards[4]|Bitboards[2]);
    occupied |= to;
    occupied &= (~from);
    pieces[0][2] = __builtin_popcountll((DiagAttacks(occupied,target)|AntiAttacks(occupied,target))&Bitboards[4]&enemy);
    occupied ^= (enemy&(Bitboards[5]|Bitboards[6]));
    occupied |= to;
    occupied &= (~from);
    pieces[0][4] += __builtin_popcountll((FileAttacks(occupied,target)|GetRankAttacks(occupied,target))&Bitboards[6]&enemy);
    occupied ^= (enemy&(Bitboards[2]|Bitboards[4]|Bitboards[5]));
    occupied |= to;
    occupied &= (~from);
    pieces[0][4] += __builtin_popcountll((DiagAttacks(occupied,target)|AntiAttacks(occupied,target))&Bitboards[6]&enemy);
    occupied ^= (Bitboards[2]|Bitboards[4]|Bitboards[6]);
    occupied |= to;
    occupied &= (~from);
    pieces[1][4] += __builtin_popcountll((DiagAttacks(occupied,target)|AntiAttacks(occupied,target))&Bitboards[6]&us);
    occupied ^= (us&(Bitboards[2]|Bitboards[4]|Bitboards[5]));
    occupied |= to;
    occupied &= (~from);
    pieces[1][4] += __builtin_popcountll((FileAttacks(occupied,target)|GetRankAttacks(occupied,target))&Bitboards[6]&us);
    if (attacker > 2) {
      pieces[1][attacker - 2]--;
    }
    int next[2] = {0, 0};
