...
const int nnuefilesize = (1538 * nnuesize + 4 * nnuesize * outputbuckets + 2 * outputbuckets);
...
void NNUE::loaddefaultnet() {
  ...
  for (int i = 0; i < 768; i++) {
    ...
    for (int j = 0; j < nnuesize; j++) {
      short int weight = 256*(short int)(NNUEData[offset+1])+(short int)(unsigned char)(NNUEData[offset]);
